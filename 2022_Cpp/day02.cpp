...
struct Rucksack
{
	...
	Play calc_own(Outcome out, Play opponent)
	{
		...
		else if(out == Outcome::DRAW) { return opponent; }
...
int main()
{
	...
	auto part1 =
		std::accumulate(games.begin(), games.end(), 0,
						[](int b, const auto &r) { return r.pointsA + b; });
	auto part2 =
		std::accumulate(games.begin(), games.end(), 0,
						[](int b, const auto &r) { return r.pointsB + b; });
